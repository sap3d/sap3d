...
class Zero123Guidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        # * multiview
        use_sc_sds        : bool = False
        cond_view         : int  = 3
        cond_image_folder : str  = "../dataset/data/train/GSO/JA_Henckels_International_Premio_Cutlery_Block_Set_14Piece"
    ...
    def configure(self) -> None:
        ...
        if self.cfg.use_sc_sds:
            self.prepare_multiview_embeddings(self.cfg.cond_image_folder)
        else:
    ...
    @torch.cuda.amp.autocast(enabled=False)
    def prepare_multiview_embeddings(self, image_folder: str) -> None:
        # * load camera pose [n_views,3,4]
        finetune_result    = torch.load(self.cfg.estimate_pose_path)
        elevation_deg_pred = torch.FloatTensor(finetune_result['elevation_pred']).to(self.device) / np.pi * 180.
        azimuth_deg_pred   = torch.FloatTensor(finetune_result['azimuth_pred']).to(self.device) / np.pi * 180.

        self.n_views = len(elevation_deg_pred)
        self.cond_elevation_deg = (90.0 - elevation_deg_pred) - min(90.0 - elevation_deg_pred)
        # self.cond_azimuth_deg   = (azimuth_deg_pred - azimuth_deg_pred[0] + 180) % 360 - 180 # [-180, 180]
        self.cond_azimuth_deg   = azimuth_deg_pred - azimuth_deg_pred[0]
        self.cond_camera_distance = torch.FloatTensor([self.cfg.cond_camera_distance] * self.n_views).to(self.device)
        
        self.view_list = list(range(self.n_views))
        # * load cond images for zero123
        img_paths = [f'{image_folder}/images/{i:03d}.png' for i in range(self.n_views)]
        rgb_256   = []
        for img_path in img_paths:
            rgbai = cv2.cvtColor(cv2.imread(img_path, cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA)
            rgbai = cv2.resize(rgbai, (256, 256), interpolation=cv2.INTER_AREA).astype(np.float32) / 255.0
            rgbi  = rgbai[..., :3] * rgbai[..., 3:] + (1.0 - rgbai[..., 3:])
            rgb_256.append(torch.from_numpy(rgbi).float())
        self.rgb_256 : Float[Tensor, "B 3 256 256"] = torch.stack(rgb_256, dim=0).permute(0, 3, 1, 2).contiguous().to(self.device)
        self.c_crossattn, self.c_concat = self.get_img_embeds(self.rgb_256)
    ...
    def get_multiview_cond(
        ...
        **kwargs,
        ...
        if len(self.view_list) == 0:
            self.view_list = list(range(self.n_views))
        total_view = len(self.view_list)
        random_index  = random.sample(list(range(total_view)), self.cfg.cond_view)
        # collate_index = random_index
        collate_index = [self.view_list.pop(i) for i in random_index]
            
        cond_elevation_deg    = self.cond_elevation_deg[collate_index]
        cond_azimuth_deg      = self.cond_azimuth_deg[collate_index]
        cond_camera_distances = self.cond_camera_distance[collate_index]
        c_crossattn           = self.c_crossattn[collate_index]
        c_concat              = self.c_concat[collate_index]

        conds = []
        # * calculate target camera pose
        for i in range(self.cfg.cond_view):
            T = torch.stack(
                [
                    torch.deg2rad((90 - elevation) - (90 - cond_elevation_deg[i:i+1])),  # Zero123 polar is 90-elevation
                    torch.sin(torch.deg2rad(azimuth - cond_azimuth_deg[i:i+1])),
                    torch.cos(torch.deg2rad(azimuth - cond_azimuth_deg[i:i+1])),
                    camera_distances - cond_camera_distances[i:i+1],
                ],
                dim=-1,
            )[:, None, :].to(self.device)
            
            clip_emb = self.model.cc_projection(torch.cat([(c_crossattn[i:i+1]).repeat(len(T), 1, 1), T], dim=-1))
            cond     = {
                "c_crossattn": [
                    torch.cat(
                        [
                            torch.zeros_like(clip_emb).to(self.device), 
                            clip_emb
                        ], 
                        dim=0)
                ],
                "c_concat": [
                    torch.cat(
                        [
                            torch.zeros_like(c_concat[i:i+1]).repeat(len(T), 1, 1, 1).to(self.device),
                            (c_concat[i:i+1]).repeat(len(T), 1, 1, 1),
                        ],
                        dim=0)
                ],
            }
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.cfg.use_sc_sds:
            loss_sds = 0.0
            conds = self.get_multiview_cond(elevation, azimuth, camera_distances)
            # add noise
            ...
            for cond in conds:
                ...
                loss_sdsi = 0.5 * F.mse_loss(latents, target, reduction="sum") / batch_size
                loss_sds += loss_sdsi
            loss_sds = loss_sds / self.cfg.cond_view
        else:
