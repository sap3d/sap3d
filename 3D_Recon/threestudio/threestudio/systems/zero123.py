...
class Zero123(BaseLift3DSystem):
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        self.validation_alpha = []
        self.validation_normal = []

    def validation_step(self, batch, batch_idx):
        out = self(batch)
        self.validation_rgb.append(out["comp_rgb"][0].cpu().numpy())
        self.validation_depth.append(out["depth"][0].cpu().numpy())
        self.validation_alpha.append(out["opacity"][0].cpu().numpy())
        self.validation_normal.append(out["comp_normal"][0].cpu().numpy())
    ...
    def on_validation_epoch_end(self):
        rgb_save_folder = os.path.join(self.get_save_dir(), 'rgb_pred')
        depth_save_folder = os.path.join(self.get_save_dir(), 'depth_pred')
        alpha_save_folder = os.path.join(self.get_save_dir(), 'alpha_pred')
        normal_save_folder = os.path.join(self.get_save_dir(), 'normal_pred')
        os.makedirs(rgb_save_folder, exist_ok=True)
        os.makedirs(depth_save_folder, exist_ok=True)
        os.makedirs(alpha_save_folder, exist_ok=True)
        os.makedirs(normal_save_folder, exist_ok=True)
        for i, (rgb_pred, depth_pred, alpha_pred, normal_pred) in enumerate(zip(self.validation_rgb, self.validation_depth, self.validation_alpha, self.validation_normal)):
            rgb_pred = Image.fromarray((np.clip(rgb_pred, 0.0, 1.0) * 255.0).astype(np.uint8))
            rgb_pred.save(f'{rgb_save_folder}/{i:03d}.png')
            depth_pred = Image.fromarray((np.clip(depth_pred[..., 0], 0.0, 1.0) * 255.0).astype(np.uint8))
            depth_pred.save(f'{depth_save_folder}/{i:03d}.png')
            alpha_pred = Image.fromarray((np.clip(alpha_pred[..., 0], 0.0, 1.0) * 255.0).astype(np.uint8))
            alpha_pred.save(f'{alpha_save_folder}/{i:03d}.png')
            
            # try:
            #     normal_pred[(normal_pred == [0, 0, 0]).all(axis=2)] = [1.0, 1.0, 1.0]
            # except:
            #     pass

            normal_pred = Image.fromarray((np.clip(normal_pred, 0.0, 1.0) * 255.0).astype(np.uint8))
            normal_pred.save(f'{normal_save_folder}/{i:03d}.png')
