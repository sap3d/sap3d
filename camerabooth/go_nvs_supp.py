...
def go_nvs(
    ...
    # HXY : for supp
    saving_dir_path = saving_dir_path + '_supp'
    ...
    # Total number of frames for the sequence
    total_frames = 360

    # Define the pattern of elevation change across the frames
    segment_frames = total_frames // 4  # Four segments for the pattern

    # Initialize the elevation array to the pattern
    delta_elevations = np.concatenate([
        np.linspace(0, -30 * np.pi / 180, segment_frames, endpoint=False),
        np.linspace(-30 * np.pi / 180, 0, segment_frames, endpoint=False),
        np.linspace(0, 30 * np.pi / 180, segment_frames, endpoint=False),
        np.linspace(30 * np.pi / 180, 0, segment_frames, endpoint=True)
    ])
    # Initialize lists to hold the new values for camera poses
    elevations_nvs, azimuths_nvs, radius_nvs = [], [], []
    ...
    # Use a loop to append the computed values, reflecting the structure of the original code
    ...
    for i in tqdm.tqdm(range(total_frames), desc=f'Novel View Synthesis for {obj_name}'):
    ...
    print(f'{save_syn_dir}/{obj_name}_sc.mp4')
...
if __name__ == '__main__':
    ...
    if log_path:
        ...
        # HXY: Use original zero123 ckpt
        ckpt_path = "/shared/xinyang/threetothreed/105000.ckpt"
        ...
        saving_dir_path  = f'experiments_nvs_zero/{opt.object_type}/{opt.object_name}_view_{opt.train_view}' # HXY: Change the saving folder here
