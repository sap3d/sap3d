...
if __name__ == '__main__':
    ...
    for train_view in train_views:
        ...
        root_folder   = f"experiments_GSO_demo_view_{train_view}"
        ...
        for ind, class_name in enumerate(class_names):
            ...
            if evaluation_folder_path:
                ...
                for i in os.listdir(evaluation_folder_path):
                    ...
                    config_path = f'/shared/xinyang/SAP3D/camerabooth/configs/GSO_demo/config_{class_name}_view_{train_view}.yaml'
                    ...
                    # elevation_pred = np.array(config_data["model"]["params"]["a_init"]) * np.pi
                    # azimuth_pred = np.array(config_data["model"]["params"]["b_init"]) * np.pi * 2.0
                    # radius_pred = np.array(config_data["model"]["params"]["c_init"]) * 0.7 + 1.5
                    ...
                    # print(class_name, train_view, elevation_pred, elevation_gt)
        ...
        print('ELEVATION')
        # print(np.mean(mean_e_beofre))
        print(np.mean(mean_e_after) * 180 / np.pi)
        print('AZIMUTH')
        # print(np.mean(mean_a_beofre))
        print(np.mean(mean_a_after) * 180 / np.pi)
        print('RADIUS')
        # print(np.mean(mean_r_beofre))
