...
def calculate_metrics(predict_path, groundtruth_path):
    ...
    for i in range(84):
        pred_image = read_and_convert_image(f'{predict_path}/rgb_pred/{i:03}.png')
...
if __name__ == '__main__':
    ...
    log_path = f'{root_dir}/3D_Recon/threestudio/{target_res}/{object_name}_ours'
    print("log_path:", log_path)
    latest_dir = find_last_log(log_path)
    predict_path = f'{latest_dir}/save'
    ...
    new_results = {
        "3d_psnr_mean": float(psnr_mean), 
        "3d_ssim_mean": float(ssim_mean),
        "3d_lpips_mean": float(lpips_mean)
    }
