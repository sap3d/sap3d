...
class ObjaverseData(Dataset):
    def __init__(self,
        ...
        self.reg_root = "/shared/xinyang/views_release"
        self.root_dir = Path(root_dir)
        ...
        with open(os.path.join(self.reg_root, 'valid_paths.json')) as f:
            self.reg_paths = json.load(f)
        
        self.total_reg_objects = len(self.reg_paths)
    ...
    def __getitem__(self, index):
        ...
        data["index_target"] = torch.tensor([index_target])
        data["index_cond"] = torch.tensor([index_cond])
        
        print("index_target: ", index_target)
        print("index_cond: ", index_cond)
        # data["T"] = self.get_T_my(theta_target, theta_cond, azimuth_target, azimuth_cond, z_target, z_cond)
        ...
        if self.reg_objaverse:
            ...
            # reg using objaverse
            random_index_tensor = torch.randint(0, self.total_reg_objects, size=(1,))
            random_index = int(random_index_tensor.item())
            total_view = 12
            index_target, index_cond = random.sample(range(total_view), 2) # without replacement
            filename = os.path.join(self.reg_root, self.reg_paths[random_index])
