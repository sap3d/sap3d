...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        a_shape, b_shape, c_shape = 3,3,3

        # Create tensors with uniform random values between 0 and 1
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        index_target = batch['index_target'].to(self.camera_poses.device)
        index_cond = batch['index_cond'].to(self.camera_poses.device)
        # print()
        # print(index_target.device)
        # print(self.camera_poses.device)
        # print(index_target)
        # print(index_cond)
        # print(self.camera_poses)

        RT_target = self.camera_poses[:, index_target]
        RT_cond = self.camera_poses[:, index_cond]
        theta_target, azimuth_target, z_target = RT_target
        theta_cond, azimuth_cond, z_cond = RT_cond
        # print(RT_target)
        # print(theta_cond)
        
        T = self.get_T_my(theta_target, theta_cond, azimuth_target, azimuth_cond, z_target, z_cond).unsqueeze(0).to(memory_format=torch.contiguous_format).float()
        # print("###")
        # print(self.device)
        # print(T.shape)
        # print(T.device)
        ...
        with torch.enable_grad():
            ...
            T = T.to(self.device)
            # print("Camera", self.camera_poses.device)
            # print("Check!")
            # print(prompt_mask)
            # print(null_prompt)
            # print(clip_emb)
            # print(self.cc_projection)
            # print(self.device)
            # print(T.device)
    ...
    def shared_step(self, batch, **kwargs):
        ...
        print(x.shape)
        print(c.keys())
        # print(c['c_crossattn'][0].shape)
        # print(c['c_concat'][0].shape)
        # pdb.set_trace()
        loss = self(x, c)
        return loss
    ...
    def configure_optimizers(self):
        ...
        params = []
        camera_pose_params = []
        camera_pose_params.extend(self.camera_poses)
        # print(self.unet_trainable)
        # print(self.unet_trainable == "attn")
        # print("###")
        ...
        #     {"params": params, "lr": lr},
        #     {"params": camera_pose_params, "lr": lr*10}
        # ])
        
        opt = torch.optim.AdamW([{"params": params, "lr": lr},
                             {"params": self.camera_poses, "lr": lr*10}], lr=lr)
        # opt = torch.optim.AdamW([{"params": self.model.parameters(), "lr": lr},
