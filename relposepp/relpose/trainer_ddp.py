...
from eval_driver import full_evaluation
from go_relpose import inference_evaluation
...
def get_parser():
    ...
    parser.add_argument("--interval_checkpoint", type=int, default=2000)
    parser.add_argument("--interval_delete_checkpoint", type=int, default=2000)
    parser.add_argument("--interval_visualize", type=int, default=1000)
    parser.add_argument("--interval_evaluate", type=int, default=10)
...
class Trainer(object):
    def __init__(self, args) -> None:
        ...
        self.eval_iter = int(5*self.interval_checkpoint)
        self.accumulate_grad_batches = 128
    ...
    def train(self):
        ...
        while self.iteration < self.num_iterations:
            for batch in self.dataloader:
                ...
                # todo add evaluation loop
                if self.rank == 0 and self.iteration % int(self.interval_checkpoint) == 0:
                    # pdb.set_trace()
                    error_info = inference_evaluation(
                    ...
                    self.writer.add_scalar(
                        "Eval/rotation_error", get_scalar_value(error_info['rotation_error_all']), self.iteration
                    )
                    self.writer.add_scalar(
                        "Eval/translation_error", get_scalar_value(error_info['translation_error_all']), self.iteration
                    )
                    self.writer.add_scalar(
                        "Eval/elevation_error", get_scalar_value(error_info['elevation_deg_error']), self.iteration
                    )
                    self.writer.add_scalar(
                        "Eval/azimuth_error", get_scalar_value(error_info['azimuth_deg_error']), self.iteration
                    )
                    self.writer.add_scalar(
                        "Eval/radius_error", get_scalar_value(error_info['radius_error']), self.iteration
                    )
