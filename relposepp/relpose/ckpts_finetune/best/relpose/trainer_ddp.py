    ...
    def train(self):
        ...
        while self.iteration < self.num_iterations:
            for batch in self.dataloader:
                ...
                # todo add evaluation loop
                if self.rank==0 and self.iteration%int(5*self.interval_checkpoint)==0:
                    ...
                    self.writer.add_scalar(
                        "Eval/rotation_error", error_info['rotation_error_all'].item(), self.iteration
                    )
                    self.writer.add_scalar(
                        "Eval/translation_error", error_info['translation_error_all'].item(), self.iteration
                    )
                    self.writer.add_scalar(
                        "Eval/elevation_error", error_info['elevation_error_all'].item(), self.iteration
                    )
                    self.writer.add_scalar(
                        "Eval/azimuth_error", error_info['azimuth_error_all'].item(), self.iteration
                    )
