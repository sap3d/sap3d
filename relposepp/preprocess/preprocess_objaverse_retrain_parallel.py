...
def process_file(name, entire_objaverse_root, num_of_imgs, focal_x, focal_y, principal_point_x, principal_point_y):
    ...
    try:
        for index in range(num_of_imgs):
            ...
            if result is not None and len(result) != 0:
                try:
                    ...
                    print(f"Error processing {image_png_path}: {e}")
                    continue
    ...
    if len(view_data) == num_of_imgs:
        return {f'{view_cls}': view_data}
    else:
        return None
    
def parallel_process(paths, entire_objaverse_root, num_of_imgs, focal_x, focal_y, principal_point_x, principal_point_y, max_workers=None):
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_file, name, entire_objaverse_root, num_of_imgs, focal_x, focal_y, principal_point_x, principal_point_y) for name in paths]
        results = []
        for future in tqdm.tqdm(concurrent.futures.as_completed(futures), total=len(paths)):
...
if __name__ == '__main__':
    ...
    results = parallel_process(objaverse_paths, entire_objaverse_root, num_of_imgs, focal_x, focal_y, principal_point_x, principal_point_y, max_workers=16)
    for result in results:
        view_release_dict.update(result)
